			...
			<div class="row pb-10">
				...
				<div class="col-xl-3 col-lg-3 col-md-6 mb-20">
					<div class="card-box height-100-p widget-style3">
						...
						$status=0;
						$statushod = 1;
						$sql = "SELECT id from tblleave where RegRemarks=:status and HodRemarks =$statushod ";
						$query = $dbh -> prepare($sql);
						$query->bindParam(':status',$status,PDO::PARAM_STR);
						$query->execute();
						$results=$query->fetchAll(PDO::FETCH_OBJ);
					...
					<table class="data-table table stripe hover nowrap">
						...
						<tbody>
							<tr>
								...
								<?php 
								$status=1;
								$sql = "SELECT tblood.id as lid,tblemployees.FirstName,tblemployees.LastName,tblemployees.emp_id,tblemployees.Gender,tblemployees.Phonenumber,tblemployees.EmailId,tblemployees.Av_leave,tblemployees.Position_Staff,tblemployees.Staff_ID,tblood.Oodtype,tblood.ToDate,tblood.FromDate,tblood.PostingDate,tblood.RequestedDays,tblood.Reason,tblood.HodRemarks,tblood.HrRemarks,tblood.HodDate,tblood.HrDate,tblood.num_days from tblood join tblemployees on tblood.emp_id=tblemployees.emp_id where tblood.HodRemarks= '$status' OR tblemployees.role = 'HOD' order by lid desc limit 5";
									$query = mysqli_query($conn, $sql) or die(mysqli_error());
									while ($row = mysqli_fetch_array($query)) {
									  $cnt=1;
								 ?>  
								...
								<td>
									<!-- <div class="dropdown">
										...
										<div>
											<a class="dropdown-item" href="ood_details.php?oodid=<?php echo $row['lid']; ?>"><i class="dw dw-eye"></i> View</a>
											<!-- <a class="dropdown-item" href="admin_dashboard.php?leaveid=<?php echo $row['lid']; ?>"><i class="dw dw-delete-3"></i> Delete</a> -->
										</div>
									<!-- </div> -->
								</td>
